...
export const getOtherUserDataRequest = async (accessToken: string, username: string) => {
    try {
        const response = await fetch(`${process.env.REACT_APP_BASE_URL}users/profile-data?username=${username}`, {
            method: "GET",
            headers: {
...
export const rateUserRequest = async (accessToken: string, rateData: RateUserData) => {
    try {
        const response = await fetch(`${process.env.REACT_APP_BASE_URL}users/rate-user`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                "Authorization": `Bearer ${accessToken}`,
