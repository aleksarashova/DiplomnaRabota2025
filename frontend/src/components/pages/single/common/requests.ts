...
export const getIsRecipeLiked = async (recipeId: string, accessToken: string) => {
    try {
        const response = await fetch(`http://localhost:8000/api/users/get-is-recipe-liked?recipeId=${recipeId}`, {
            method: "GET",
            headers: {
                "Content-Type": "application/json",
                "Authorization": `Bearer ${accessToken}`,
...
export const addRecipeToLiked = async (recipeId: string, accessToken: string) => {
    try {
        const response = await fetch("http://localhost:8000/api/users/add-recipe-to-liked", {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                "Authorization": `Bearer ${accessToken}`,
